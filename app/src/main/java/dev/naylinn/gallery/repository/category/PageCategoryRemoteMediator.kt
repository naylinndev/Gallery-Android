...
import dev.naylinn.gallery.common.TYPE_CATEGORY
...
class PageCategoryRemoteMediator(
    ...
    override suspend fun load(
        ...
        try {
            ...
            val loadKey = when (loadType) {
                ...
                LoadType.APPEND -> {
                    ...
                    val remoteKey = appDatabase.withTransaction {
                        remoteKeyDao.getRemoteKeyByType(type = TYPE_CATEGORY)
                    }
            ...
            val data = netApi.getCategory(appSecret = APP_SECRET, page = loadKey)
            data
                .onSuccess { response ->
                    val result = response.body()!!
                    val categoryEntities = result.data.map { it.mapToRoomEntity() }
                    isLastPage = !result.hasMorePages
                    appDatabase.withTransaction {

                        if (loadType == LoadType.REFRESH) {
                            remoteKeyDao.deleteByType(type = TYPE_CATEGORY)
                        }
                        ...
                        categoryDao.saveCategories(categoryEntities = categoryEntities)
